...
// APP RTL
gulp.task('styles:app:rtl', ['styles:bs:rtl'], function() {
...
// Bootstrap to RTL
gulp.task('styles:bs:rtl', function() {
    ...
    return gulp.src(config.source.templates.views)
        ...
        .pipe($.useref({
            ...
            transformPath: function(filePath) {
                return filePath.replace('vendor/', 'node_modules/')
            }
        }))
        .on('error', handleError)
...
gulp.task('vendor', function() {
    ...
    return gulp.src(config.source.vendor, { base: config.paths.npm })
        .pipe(gulp.dest(config.build.vendor));
});
